...
def check_history(penguin_name: str) -> Dict[str, Any]:
    ...
    Args:
        penguin_name (str): The name of the penguin whose history to check.

    Returns:
        Dict[str, Any]: A dictionary containing:
            - recent_food (int): Sum of the last 3 food distributions.
            - has_tool (bool): Whether the penguin has received a tool before.
    """
...
def record_distribution(penguin_name: str, food: int, has_tool: bool) -> str:
    """
    Record the resource distribution for a penguin.

    Args:
        penguin_name (str): The penguin's name.
        food (int): Units of food given.
        has_tool (bool): Whether a tool was given.

    Returns:
        str: Confirmation message describing the recorded distribution.
    """
...
def find_food(penguin_name: str, method: str) -> int:
    """
    Make a penguin gather food using a specific method.

    Args:
        penguin_name (str): Name of the penguin performing the action.
        method (str): Either "fishing" (high yield) or "foraging" (low yield).
    ...
    if method == "fishing":
        # Penguins catch more food while fishing
        amount = random.randint(5, 15)
    elif method == "foraging":
        # Less efficient than fishing
        amount = random.randint(1, 6)
    else:
        raise ValueError(
            f"Unknown method '{method}'. Valid methods are 'fishing' or 'foraging'."
        )

    print(f"{penguin_name} used '{method}' and found {amount} units of food.")
    return amount
...
class PenguinAgent(ToolCallingAgent):
    ...
    def take_action(self) -> Dict[str, Any]:
        ...
        # Decision logic BEFORE asking model (to avoid hallucinations)
        if self.food < 2:
            decision = {"action": "request_food", "method": "none"}
        else:
            if self.has_tool:
                decision = {"action": "find_food", "method": "fishing"}
            else:
                decision = {"action": "find_food", "method": "foraging"}

        prompt = f"""You are Penguin {self.name}.
        Your current state:
        - Food: {self.food}
        - Has tool: {self.has_tool}
        Emit exactly one JSON object reflecting this decision:
        {json.dumps(decision)}"""
